...
class Environment:
    ...
    def list_things_at(self, location, tclass=Thing):
        ...
        if isinstance(location, numbers.Number):
            return [print(thing) for thing in self.things
...
class VacuumEnvironment(XYEnvironment):
    ...
    def add_obstacle_custom(self):
        for i in range(5):
            for j in range(1,4):
                self.add_thing(Obstacle(), (i,j))
        for i in range(5,9):
            for j in range(1,4):
                self.add_thing(Obstacle(), (i,j))
    def print(self):
        for i in range(self.width):
            for j in range(self.height):
                print((i,j),self.list_things_at((i,j), Obstacle))
    def percept(self, agent):
...
"""
The classes declared previously were taken from the AIMA-python repository
"""

"""
A simple reflex agent for the Figure 2.8 can be represented with the following class
(lines from 194 to 228 of https://github.com/aimacode/aima-python/blob/master/agents.py)
"""
loc_A, loc_B = (0, 0), (1, 0)
